...
print("UA inflation is a float64 which is a real number")
...
# Résumé temporel
start = infl_panel.index.min().strftime("%Y-%m")
end   = infl_panel.index.max().strftime("%Y-%m")
freq  = pd.infer_freq(infl_panel.index)

# Tableau par pays (propre)
stats_by_country = pd.DataFrame({
    "Start": start,
    "End": end,
    "Freq": freq,
...
# Ordre des colonnes
stats_by_country = stats_by_country[["Start", "End", "Freq", "Mean", "Std", "N"]]

# Export LaTeX propre
print(
    stats_by_country.to_latex(
        float_format="%.2f",
        caption="Inflation summary by country (sample period, frequency, mean and standard deviation)",
        label="tab:inflation_summary",
        column_format="lccc rr r",
        bold_rows=False
    )
)
...
corr_ua_fr_pct = infl_panel["UA"].corr(infl_panel["FR"]) * 100

print(f"Correlation between UA and FR inflation: {corr_ua_fr_pct:.2f}%")

corr_fr_de_pct = infl_panel["FR"].corr(infl_panel["DE"]) * 100

print(f"Correlation between FR and DE inflation: {corr_fr_de_pct:.2f}%")

#5/ Test for corr significance
...
print("UA–FR correlation:")
print(f"  Correlation (%) = {corr_ua_fr * 100:.2f}%")
...
print("\nFR–DE correlation:")
print(f"  Correlation (%) = {corr_fr_de * 100:.2f}%")
...
def break_tests(series: pd.Series, name: str):
    s = series.dropna().copy()
    if len(s) < 30:
        print(f"{name}: not enough observations ({len(s)})")
        return

    # Time trend
    t = pd.Series(range(len(s)), index=s.index, name="trend")
    X = add_constant(t)

    # Regression
    res = OLS(s.values, X.values).fit()

    # CUSUM test
    _, cusum_p, _ = breaks_cusumolsresid(
        res.resid, ddof=int(res.df_model)
    )

    # Simple output
    result = "STRUCTURAL BREAK" if cusum_p < 0.05 else "NO STRUCTURAL BREAK"

    print(f"{name}: p-value = {cusum_p:.4f} → {result}")


# Run for UA and FR
break_tests(infl_panel["UA"], "UA inflation")
break_tests(infl_panel["FR"], "FR inflation")

#7/ How persistant is inflation in UA ? in FR ? just use ADF
...
def adf_persistence(series, name):
    ...
    if pval < 0.05:
        result = "LOW persistence (stationary)"
    else:
        result = "HIGH persistence (unit root)"
