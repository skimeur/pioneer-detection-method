...
- `pdm.py` – implementation of all PDM methods
- `exercise_pdm_inflation.py` – applied exercise on European inflation dynamics  
...
```
pdm.py                         # PDM implementation (all 8 methods)
exercise_pdm_inflation.py      # Applied exercise — European inflation / Ukraine
part_B_counterfactual.py       # Exam Part B — counterfactual SVAR pipeline
part_A_monetary_regime.md      # Exam Part A — NBU regime chronology
part_B_interpretation.md       # Exam Part B — interpretation paragraph
data_ecb_hicp_panel.csv        # ECB HICP panel (11 EA countries, 2000–2025)
data_ukraine_cpi_raw.csv       # Ukraine CPI MoM index (SSSU, 2005–2025)
data_nbu_uahusd.csv            # UAH/USD daily (NBU API, auto-cached)
data_wb_ukraine_gdp.csv        # Ukraine real GDP growth (World Bank, auto-cached)
...
## 5. Example — Applied Exercise on European Inflation

`exercise_pdm_inflation.py` applies PDM and all benchmark methods to a panel of European HICP inflation series (11 eurozone countries + Ukraine, 2000–2025).

**Outputs** (auto-saved on run):
- `figures/fig1_pioneer_weights_lines.png` — pioneer weights over time
- `figures/fig2_pioneer_weights_heatmap.png` — heatmap by country and subperiod
- `figures/fig3_dominant_pioneer.png` — rolling dominant pioneer
- `tables/avg_weights_by_subperiod.csv` — average weights by country and period
- `tables/rmse_by_method.csv` — RMSE of each method for predicting Ukraine inflation
...
| `part_B_counterfactual.py` | Part B deliverable — full econometric pipeline (run this) |
| `part_B_interpretation.md` | Part B deliverable — standalone interpretation paragraph |
| `figures/ukraine_counterfactual_inflation.png` | **Primary output figure** (exam deliverable) |
| `figures/ukraine_counterfactual_robustness.png` | Supplementary — all methods comparison |
